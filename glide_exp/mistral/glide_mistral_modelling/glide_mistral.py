...
class LigerMistralAttention(nn.Module):
    def __init__(
        self, 
        config: GlideMistralConfig,
...
class GlideMistralDecoderLayer(MistralDecoderLayer):
    def __init__(self, config: GlideMistralConfig, layer_idx: int, window_size: int = 64):
        super().__init__(config, layer_idx) 
        self.hidden_size = config.hidden_size
        self.self_attn = LigerMistralAttention(config=config, layer_idx=layer_idx, window_size=window_size)
        self.mlp = MistralMLP(config)
        self.input_layernorm = MistralRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MistralRMSNorm(config.hidden_size, eps=config.rms_norm_eps)


class GlideMistralPreTrainedModel(MistralPreTrainedModel):
    config_class = GlideMistralConfig
...
class GlideMistralModel(MistralModel, GlideMistralPreTrainedModel):
    def __init__(self, config: GlideMistralConfig):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [
                GlideMistralDecoderLayer(config, layer_idx,
                    window_size=config.layer_delta_configuration.get(layer_idx, {}).get("window_size", config.global_window_size))
...
class GlideMistralForCausalLM(MistralForCausalLM, GlideMistralPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}

    def __init__(self, config: GlideMistralConfig):
        super().__init__(config)
        self.model = GlideMistralModel(config)
